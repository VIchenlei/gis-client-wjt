...
export default class CardStore {
  ...
  cardLeave(data) {
    ...
    for (let i = 0, len = rows.length; i < len; i++) {
      ...
      let cardTypeID = card[1]
      let cardType = `${cardTypeID === 2 ? 'v' : 's'}cards`
      if (this[cardType] && this[cardType].has(cardID)) { //  只有人员有升级消息
  ...
  processStaffData(data, needMove) {
    ...
    // if (xdata.depts || xdata.objRange === 1 || xdata.isCheck === 1)
     this.constructData('staff')
  ...
  processStat(data, type) {
    ...
    if (data) {
      ...
      if (xdata.isCheck === 1) { // 检查用户登区域人数修改为过滤黑名单后的人员总数
        let cards = this[`${type ==='staff' ? 's' : 'v'}cards`]
        cards = cards && Array.from(cards.values())
        stat = this.processStaffNumber(stat, cards)
      }
    }
  ...
  // 检查用户登录 区域、部门、职务人数修改为过滤黑名单后的人数
  processStaffNumber(stat, cards) {
    let totalLength = 0
    let statNames = ['area', 'dept', 'occupation_level']
    statNames.forEach(name => {
      let datas = stat[name]
      datas.forEach(data => {
        let persons = cards.filter(item => item[CARD[`${name}_id`]] === data[0])
        data[1] = persons ? persons.length : data[1]
        if (name === 'area') totalLength += data[1]
      })
      stat[name] = datas.filter(data => data[1] !== 0) // 过滤黑名单后人数为0 实时左侧列表隐藏对应部门、区域、职务
    })
    stat['sum'] = totalLength
    return stat
  }
